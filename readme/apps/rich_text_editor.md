...
At its core, Joplin stores notes in [Markdown format](https://github.com/dpjl/joplin/blob/dev/readme/apps/markdown.md). Markdown is a simple way to format text that looks great on any device and, while it's formatted text, it still looks perfectly readable in a plain text editor.
